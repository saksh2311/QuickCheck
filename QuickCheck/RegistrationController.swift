...
class RegistrationController: UIViewController {
    ...
    let backToLoginButton : UIButton = {
        let button = UIButton()
        button.translatesAutoresizingMaskIntoConstraints = false
        button.setTitle("< Back to Login", for: UIControl.State.normal)
    ...
    func setupViews(){
        ...
        let registerButtonView = registerButton.getStyledButtonView()
        baseStackView.addArrangedSubview(registerButtonView)
        
        let backToLoginButtonView = backToLoginButton.getStyledButtonView()
    ...
    func checkUserInput() -> Bool{
        ...
        else if !userID.contains("@qr.com"){
            isError = true
            errorMessage = "UserID must be a valid email ID. Make sure it contains @qr.com suffix"
        }
    ...
    func registerUser() {
        ...
        Auth.auth().createUser(withEmail: email, password: password) { (authResult, error) in
            ...
            if let error = error {
                ...
                self.showAlert(AlertTitle: "Registration Failed", Message: error.localizedDescription)
                return
            }
            
            guard let userID = authResult?.user.uid else {
                UIViewController.removeSpinner(spinner: loadingScreen)
                self.showAlert(AlertTitle: "Registration Failed", Message: "Could not create user ID")
                return
            }
            ...
            ref.child(userType).child(userID).setValue(userDetails) { (error, ref) in
                if let error = error {
                    print("Error creating user database: \(error)")
                    UIViewController.removeSpinner(spinner: loadingScreen)
                    self.showAlert(AlertTitle: "Registration Failed", Message: "Failed to save user data")
                    return
                }
